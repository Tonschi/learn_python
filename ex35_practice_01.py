from sys import exit

def display_choices(choices_list):
    for i in range(0,len(choices_list)):
        print(">>> ", choices_list[i])

def gold_room():
    print("This room is full of gold. How much do you take?")

    choice = input("> ")
    if "0" in choice or "1" in choice:
        how_much = int(choice)
    else:
        dead("Manm kearn to type a number.")

    if how_much < 50:
        print("Nice, you are not greedy, you win!")
        exit(0)
    else:
        dead("You greedy bastard!")

def bear_room():
    print("There is a bear here. He has honey.")
    print("The fat bear is in front of another door.")
    print("How are you going to move the bear?")
    bear_moved = False
    choices = ["Take honey", "Taunt bear", "Open door"]

    display_choices(choices)

    while True:
        choice = input("> ").lower()

        if choice == "take honey":
            dead("The bear looks at you and slaps your face off.")
        elif choice == "taunt bear" and not bear_moved:
            print("The bear moved away from the door.")
            print("You can go through now.")
            bear_moved = True
        elif choice == "taunt bear" and bear_moved:
            dead("The bear gets pissed and chews your leg off.")
        elif choice == "open door" and bear_moved:
            gold_room()
        else:
            print("I got no idea what that means.")

def cthulhu_room():
    print("Now you meet the great evil cthulhu.")
    print("You'll probably lose your mind now.")
    print("Do you flee for your life or eat your own head?")
    choice = input("> ")

    if "flee" in choice:
        start()
    elif "head" in choice:
        dead("Well, that was tasty.")
    else:
        cthulhu_room()

def dead(why):
    print(why, "Good job!")
    exit(0)

def start():
    print("You are ina dark room.")
    print("THere is a door to your right and left.")
    print("Which one do you take?")

    choices = ["Left", "Right", "...?"]
    display_choices(choices)

    choice = input("> ").lower()

    if choice == "left":
        bear_room()
    elif choice == "right":
        cthulhu_room()
    else:
        dead("You stumble around the room until you starve.")


start()
